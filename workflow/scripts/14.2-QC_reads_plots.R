...
log <- file('logs/14-NRF_fastq/QCsummary_14.2.log', open = "wt")
...
for (i in 1:length(rows)) {
  ...
  plot["treatment",] <- metadata$Group[match(metadata$SampleName, gsub('X','',colnames(plot)))]
  ...
  plot$group <- factor(plot$group, levels = c("Raw CT_NSD", "Raw CT_SD", "Raw FV_NSD", 
                                              "Raw FV_SD", "Raw FT_NSD", "Raw FT_SD", 
                                              "Filtered CT_NSD","Filtered CT_SD","Filtered FV_NSD",
                                              "Filtered FV_SD","Filtered FT_NSD","Filtered FT_SD"))
...
hist_data$group <- metadata[match(gsub("X",'',rownames(hist_data)),metadata$SampleName),"Group"]

cat("Generating histogram diagnostics\n")
png(paste0(opt$outdir,"/QC_plot_density.png"))
ggplot(hist_data, aes(x = reads, col=group, fill=group)) + 
  geom_histogram(aes(alpha = .25)) +
