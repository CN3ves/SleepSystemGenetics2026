...
alignQC <- function(logs) {
  log_summary <- data.frame("File"="File", "Reads" = "Reads", "Unpaired" = "Unpaired", "Unaligned" = "Unaligned",  "Aligned 1x" = "Aligned 1x", "Multialigned" = "Multialigned", "Alignment rate" = "Alignment rate", stringsAsFactors = FALSE)
  
  # Extract data from logs
  for (i in 1:length(logs)) {
    log <- read.delim(logs[i], header=FALSE, stringsAsFactors = FALSE)
    log <- data.frame("File"= gsub(".*/(.*).fq.*", "\\1", log[grep('Aligning',log[,1]),1]),
  ...
  log_summary[1,] <- c("", apply(log_summary[-1,-1], 2, function(col) {
    ...
    col <- paste0("Min: ", col["Min."], "%; Max: ", col["Max."], "%; Mean: ", round(col["Mean"],0),"%")
