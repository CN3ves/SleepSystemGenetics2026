...
log <- file('logs/14-NRF_fastq/QCsummary_14.1.log', open = "wt")
...
source("workflow/scripts/2.0-QC_reads_helper.R")
...
idx <- as.numeric(summary_tab["total_reads (before filtering)",-1]) < 30e6
idx <- summary_tab["total_reads (before filtering)",c(FALSE,idx), drop=FALSE]
san_tab <- metadata[metadata$SampleName %in% as.numeric(gsub("_.*","",names(idx))), c("Strain", "SampleName", "Treatment", "SampleShortName")]
