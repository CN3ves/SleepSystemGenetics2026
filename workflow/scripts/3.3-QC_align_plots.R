...
log <- file('logs/3-BXD_bam/alignQC_3.3.log', open = "wt")
...
names(qcs) <- gsub(".RData", "", basename(qc_data))
...
id <- gsub('_.*','',chrs$sample)
stopifnot(all(metadata$Sample[match(id, metadata$Sample)] == id)) 
chrs$group <- metadata$Treatment[match(id, metadata$Sample)]
...
id <- gsub('_.*','',bneck$sample)
stopifnot(all(metadata$Sample[match(id, metadata$Sample)] == id)) 
bneck$group <- metadata$Treatment[match(id, metadata$Sample)]
...
id <- gsub('_.*','',tsse_line$sample)
stopifnot(all(metadata$Sample[match(id, metadata$Sample)] == id)) 
tsse_line$group <- metadata$Treatment[match(id, metadata$Sample)]
...
id <- gsub('_.*','',tsse_box$sample)
stopifnot(all(metadata$Sample[match(id, metadata$Sample)] == id))
tsse_box$group <- metadata$Treatment[match(id, metadata$Sample)]
...
id <- gsub('_.*','',rownames(plot))
stopifnot (all(metadata$Sample[match(id, metadata$Sample)] == id))  
plot$group <- metadata$Treatment[match(id, metadata$Sample)]
plot$strain <- metadata$Strain[match(id, metadata$Sample)]
plot$strain[is.na(plot$strain)] <- "test"

png(paste0(opt$outdir,"/QC_bam_pca.png"))
ggplot(plot, mapping = aes(x = PC1, y = PC2, shape=strain, color=group)) +
  geom_point()  + 
...
id <- gsub('_.*','',colnames(mcov))
stopifnot(all(metadata$Sample[match(id, metadata$Sample)] ==id))
groups <- metadata$Treatment[match(id, metadata$Sample)]
strains <- metadata$Strain[match(id, metadata$Sample)]

pcor <- cor(mcov, method = "spearman")

gcols <- rainbow(length(unique(groups)))
names(gcols) <- unique(groups)
scols <- rainbow(length(unique(strains)))
names(scols) <- unique(strains)
names(scols)[is.na(names(scols))] <- "test"

ha = HeatmapAnnotation(
    Groups = groups,
    Strains = strains,
    col = list(Groups=gcols, Strains=scols),
