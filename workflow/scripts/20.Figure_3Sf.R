# Script to produce figure S3c

# Redirect all R logs to Snakemake log
log <- file('logs/20-Figures/figure3Sc.log', open = "wt")
...
diff <-read.xlsx(opt$S2,'Differential_Expression')
...
cat("Save plot\n")
svg(paste0(opt$outdir,"/FigS3f.svg"))

mds <-plotMDS(counts[sigs,], col =  colors[strains], pch = points, top=500, gene.selection="common", main=paste("RNA PCA using top 500 significant features"))
...
write.csv(tab, paste0(opt$outdir, "/data/S3f.csv"))
